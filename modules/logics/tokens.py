    ...
    def precedence(expression: str) -> Dict:
        ...
        while i in range(len(exp)):
            if exp[i] ==  '(':
                ...
                parenthesis_count -= 1
            elif exp[i] in Tokenizer.operators and parenthesis_count == 0:
                if Tokenizer.operators[exp[i]] > max_weight:
                    max_weight = Tokenizer.operators[exp[i]]
        ...
        if current_operator == '!':
            ...
            result[current_operator] = [Tokenizer.precedence(operand)]
            
        else:
            left_exp = exp[:current_operator_index].strip()
            right_exp = exp[current_operator_index + 1:].strip()
        
            result[current_operator] = [Tokenizer.precedence(left_exp), Tokenizer.precedence(right_exp)]
